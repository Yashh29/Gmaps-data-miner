...
if run_button:
    ...
    if not queries:
        ...
        if not all_data:
            st.error("No leads were extracted. Try different keywords.")
            st.stop()
...
if st.session_state.final_df is not None:
    ...
    if "source_query" in final_df.columns:
        query_options += sorted(final_df["source_query"].dropna().unique())
    ...
    area_options = ["All Areas"] + sorted(final_df["area"].dropna().unique())
    ...
    st.download_button(
        "Download Campaign Emails",
        email_df.to_csv(index=False).encode("utf-8"),
        "campaign_emails.csv",
        "text/csv"
    )
    ...
    st.download_button(
        "Download No Website Leads",
        no_website_df.to_csv(index=False).encode("utf-8"),
